...
export const TrialAlert = memo(function TrialAlert({ trialEndsAt, userCreatedAt }: TrialAlertProps) {
  ...
  return (
    <div className={`border-l-4 p-4 mb-6 rounded-lg transition ${
      isUrgent
      ...
      <div className="flex items-start justify-between gap-2">
        ...
        <button
          ...
          className={`${isUrgent ? 'text-red-600 hover:text-red-900' : 'text-amber-600 hover:text-amber-900'} transition flex-shrink-0`}
        >
          <X className="h-5 w-5" />
        </button>
      </div>
